...
Skipped unless ``FORESEE_TEST_POSTGRES_*`` env vars are set. See
...
runs don't collide. Only the kinds of bugs that are dialect-specific (server
catalog quirks, type coercion, driver wiring) belong here; logic that is
dialect-agnostic stays in ``test_sql_loader.py``.
"""
...
@pytest.fixture
def seeded_table(postgres_config: LiveDbConfig):
    connection = postgres_config.to_connection()
    engine = build_engine(connection, postgres_config.password)
    table = unique_table_name()
    try:
        _seed(engine, table)
        yield connection, postgres_config.password, table
    finally:
        try:
            _drop(engine, table)
        finally:
...
def test_test_connection_against_live_postgres(
    postgres_config: LiveDbConfig,
) -> None:
    connection = postgres_config.to_connection()
    result = run_connection_test(connection, postgres_config.password)
    assert result["ok"] is True, result.get("error")
    assert isinstance(result["latency_ms"], int)
    assert (result.get("server_version") or "").lower().startswith("postgresql")
...
@pytest.mark.integration
def test_materialize_writes_parquet(
