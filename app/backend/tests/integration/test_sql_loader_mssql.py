...
Skipped unless ``FORESEE_TEST_MSSQL_*`` env vars are set. See
...
``importorskip`` chooses whichever is available so the suite can run on
either OS as long as the matching driver package is installed.
"""
...
@pytest.fixture
def seeded_table(mssql_config: LiveDbConfig):
    connection = mssql_config.to_connection()
    engine = build_engine(connection, mssql_config.password)
    table = unique_table_name()
    try:
        _seed(engine, table)
        yield connection, mssql_config.password, table
    finally:
        try:
            _drop(engine, table)
        finally:
...
def test_test_connection_against_live_mssql(mssql_config: LiveDbConfig) -> None:
    connection = mssql_config.to_connection()
    result = run_connection_test(connection, mssql_config.password)
...
def test_materialize_writes_parquet(seeded_table, tmp_path: Path) -> None:
    """MSSQL exercises the TOP-N wrapping branch in _wrap_with_limit."""
