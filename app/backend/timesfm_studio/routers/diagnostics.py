...
async def run_diagnostics(
    ...
    cached = store.cache_get(request.dataset_id, "diagnostics", key)
    if cached:
    ...
    store.cache_put(request.dataset_id, "diagnostics", key, result)
    return result
...
async def run_preflight(
    ...
    key = hash_params(request.model_dump())
    cached = store.cache_get(request.dataset_id, "preflight", key)
    if cached:
        return cached["result"]
    ...
    store.cache_put(request.dataset_id, "preflight", key, result)
    return result
