...
async def stl_decompose(
    ...
    cached = store.cache_get(request.dataset_id, "stl", key)
    if cached:
    ...
    store.cache_put(request.dataset_id, "stl", key, result)
    return result
...
async def segments_compare(
    ...
    key = hash_params(request.model_dump())
    cached = store.cache_get(request.dataset_id, "segments", key)
    if cached:
        return cached["result"]
    ...
    store.cache_put(request.dataset_id, "segments", key, result)
    return result
