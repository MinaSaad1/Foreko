...
_CATEGORICAL_UNIQUE_FRACTION: float = 0.10
...
def _infer_column_dtype(series: pd.Series) -> ColumnDType:
    ...
    try:
        ...
        if coerced.notna().mean() >= _DATETIME_COERCION_THRESHOLD:
            return "datetime"
    except Exception:
        pass
    nunique = series.nunique(dropna=True)
    if nunique > 0 and nunique <= max(_CATEGORICAL_MAX_UNIQUE, int(len(series) * _CATEGORICAL_UNIQUE_FRACTION)):
        return "categorical"
    return "string"
