...
def ensure_model(model_id: str, local_dir: Path) -> Path:
    ...
    ``local_dir`` is a Foreko-owned directory (not HuggingFace's default hub
    ...
    try:
        ...
        mark_error(
            ...
            f"you can place the snapshot at {local_dir} and restart Foreko."
        )
        raise
    except (ConnectionError, TimeoutError) as exc:
        mark_error(
            f"Network error reaching HuggingFace: {exc}. If this persists, "
            f"place the snapshot at {local_dir} and restart Foreko."
        )
        raise
