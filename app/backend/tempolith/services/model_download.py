...
# If no tqdm update lands within this window the cached speed is treated as
# stale and reported as zero. Lets the frontend stall detector trip and keeps
# the displayed rate honest when the connection pauses.
_SPEED_STALENESS_SECONDS = 3.0
...
def progress_snapshot() -> dict[str, object]:
    ...
    with _lock:
        now = time.monotonic()
        elapsed = max(now - _state.started_at, 0.0) if _state.started_at else 0.0
        speed = _state.speed_bps
        if _state.updated_at and (now - _state.updated_at) > _SPEED_STALENESS_SECONDS:
            speed = 0.0
        remaining = 0.0
        if (
            _state.total_bytes
            ...
            remaining = (_state.total_bytes - _state.downloaded_bytes) / speed
        return {
            "state": _state.state,
...
def ensure_model(model_id: str, local_dir: Path) -> Path:
    ...
    ``local_dir`` is a Tempolith-owned directory (not HuggingFace's default hub
    ...
    try:
        ...
        mark_error(
            ...
            f"you can place the snapshot at {local_dir} and restart Tempolith."
        )
        raise
    except (ConnectionError, TimeoutError) as exc:
        mark_error(
            f"Network error reaching HuggingFace: {exc}. If this persists, "
            f"place the snapshot at {local_dir} and restart Tempolith."
        )
        raise
...
class _ProgressTqdm(_tqdm_base):
    ...
    ``huggingface_hub`` spawns two kinds of tqdms inside ``snapshot_download``:
    a per-file *byte* counter (``unit="B"``) and a *file-count* counter
    (default ``unit="it"``, ``total`` = number of files in the repo). Only the
    byte counters represent real download progress; the file-count tqdm would
    otherwise pin ``total_bytes`` to the file count (e.g. 4) and make the
    percentage explode the moment a single file's bytes start streaming.
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        # Capture unit / total / initial from kwargs BEFORE delegating to
        # tqdm.__init__: ``disable=True`` makes tqdm short-circuit and skip
        # setting most attributes (including ``self.unit``), so reading them
        # afterwards is unreliable.
        unit = kwargs.get("unit", "it")
        self._tracks_bytes = unit == "B"
        initial_n = int(kwargs.get("initial") or 0)
        total = int(kwargs.get("total") or 0)
        kwargs["disable"] = True  # suppress visual rendering
        super().__init__(*args, **kwargs)
        if not self._tracks_bytes:
            return
        if total or initial_n:
            with _lock:
                _state.total_bytes += total
                # Credit bytes that were already on disk before this tqdm
                # started, so resumed downloads don't reset visible progress.
                _state.downloaded_bytes += initial_n
    ...
    def update(self, n: int = 1) -> bool | None:  # type: ignore[override]
        if self._tracks_bytes and n:
            with _lock:
                ...
                dt = now - _state.updated_at if _state.updated_at else 0.0
                if dt > 0.5:
                    _state.speed_bps = 0.7 * _state.speed_bps + 0.3 * (n / dt)
                _state.updated_at = now
