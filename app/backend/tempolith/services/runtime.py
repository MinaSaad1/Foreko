"""Runtime metadata for packaged Tempolith builds.
...
earlier Tempolith instance). We:

1. Pick a free port (preferring 8000 if available, else the next open one).
2. Write ``runtime.json`` under ``~/.tempolith`` so the shell can read it.
    ...
    if lock_path.exists():
        try:
            ...
            if existing != my_pid and _pid_alive(existing):
                logger.warning("Another Tempolith instance appears to be running (pid=%s).", existing)
                return False
        except Exception:
            pass
