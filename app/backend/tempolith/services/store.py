...
from tempolith.services.migrations import run_migrations


AnalysisKind = Literal[
    "backtest", "calibration", "diagnostics", "preflight",
    "anomaly_methods", "changepoints", "stl", "segments",
]
...
class Store:
    ...
    def cache_lookup(
        ...
        """Hash request params and return (key, cached_result | None).

        Callers pass the key to cache_put if they compute a fresh result.
        """
        key = hash_params(request.model_dump())
        cached = self.cache_get(dataset_id, kind, key)
        return key, cached["result"] if cached else None
