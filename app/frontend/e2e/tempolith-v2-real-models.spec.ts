 ...
 *
 * The deterministic journey in tempolith-v2-project.spec.ts runs two instant
...
const API =
  process.env.TEMPOLITH_E2E_API ?? process.env.TEMPOLITH_E2E_BASE_URL ?? "http://localhost:8002";

// Real fits, no stand-ins. TimesFM only where a checkpoint is already cached.
const MODELS = (
  process.env.TEMPOLITH_E2E_MODELS ?? "lightgbm,seasonal_naive,ets,arima,prophet"
)
  .split(",")
  .map((m) => m.trim())
  .filter(Boolean);
...
test.beforeEach(async ({ context }) => {
  await context.addInitScript(() => {
    try {
      window.localStorage.setItem("tempolith:tour:completed", "1");
    } catch {
      /* ignore */
    }
  });
