...
export function LoadingSplash() {
 ...
 <img
 src="/tempolith-logo.png"alt=""aria-hidden="true"className="relative z-10 h-24 w-24 object-contain drop-shadow-[0_0_20px_rgb(var(--color-accent)/0.35)] animate-pulse-slow"
 />
 <span className="text-2xl font-display font-semibold text-text-primary relative z-10 tracking-[0.18em] uppercase">
 Tempolith Setup
 </span>
 ...
 <button
 type="button"onClick={() => retryMutation.mutate()}
 disabled={retryMutation.isPending}
 className="btn-terminal"
 >
 {retryMutation.isPending ?"Retrying..." :"Try again"}
 </button>
 <div className="rounded-panel border border-border/40 bg-bg-surface px-4 py-3 max-w-sm text-left space-y-1">
 ...
 ~/.tempolith/models/google--timesfm-2.5-200m-pytorch/
 </code>
 )}
 then restart Tempolith.
 </p>
 </div>
 </div>
 ) : (
 ...
 <div className="h-2 w-full border border-border/60 bg-bg-surface overflow-hidden">
 <div
 className="h-full bg-accent transition-all duration-500"style={{ width: pct !== null ? `${pct}%` :"20%" }}
 />
 </div>
