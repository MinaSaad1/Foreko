...
export function ConnectionList({ onSelect, refreshToken = 0 }: ConnectionListProps) {
 ...
 return (
 <ul className="divide-y divide-border/60 border border-border/60 bg-bg-surface">
 ...
 <button
 type="button"onClick={() => onSelect(connection)}
 className="border border-accent/60 bg-accent/20 px-3 py-1.5 text-xs text-accent"
 >
 Connect
 </button>
 <button
 type="button"onClick={() => remove(connection.id)}
