...
export function Tour({ force = false, onClose }: TourProps) {
 ...
 return (
 <div
 role="dialog"aria-modal="true"aria-labelledby="tour-title"className="fixed inset-0 z-[60] flex items-center justify-center bg-bg-base/80 backdrop-blur-xl px-4"
 >
 ...
 aria-label="Skip tour"className="font-mono text-[10px] uppercase tracking-widest text-text-muted hover:text-text-primary"
 >
 Skip
 </button>
 </div>
 ...
 <button
 type="button"onClick={() => setStep((s) => Math.max(s - 1, 0))}
 disabled={isFirst}
 ...
 <button
 type="button"onClick={() => setStep((s) => Math.min(s + 1, STEPS.length - 1))}
 className="btn-terminal-primary"
 >
 Next →
 </button>
