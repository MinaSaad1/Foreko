...
All notable changes to Tempolith are documented here. Format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) and this project adheres to semantic versioning.
...
Tempolith becomes a forecasting workflow rather than a collection of analysis
...
- Known-future factor plans. A factor the selected model reads must have a value for every future period, or an explicit fill policy; every filled value is recorded with the run. Tempolith never invents one.
...
- **ETS never worked.** It called `.fit(disp=False)`, which statsmodels' `ExponentialSmoothing` does not accept, so it raised on every call and silently returned seasonal naive's forecast under the ETS name. Every "ETS" number Tempolith has shown was seasonal naive.
...
### Added
- Dataset TTL janitor: a startup-plus-hourly sweep deletes uploads older than `TEMPOLITH_DATASET_TTL_HOURS` (default 720, 30 days). Set the value to `0` to keep datasets forever. Previously the setting was advertised but nothing swept, so `~/.tempolith/datasets/` grew without bound.
...
- Default dataset retention raised from 24 hours to 30 days (`TEMPOLITH_DATASET_TTL_HOURS`, default 720). Surfaced on the Datasets and Privacy pages.
- Privacy page rewritten with a complete inventory of what Tempolith writes to `~/.tempolith/`.
...
- README and Privacy page corrected to point at `~/.tempolith/models/` (not `~/.cache/huggingface/hub/`).
...
### Security
- Dataset and adapter ids from the URL are validated as single path segments before being joined onto the storage directory, closing a Windows directory-escape where a crafted id (`..%5C..%5C...`) could read or delete files outside `~/.tempolith/`.
...
- Per-request inference timeout (`TEMPOLITH_INFERENCE_TIMEOUT_S`, default 600 seconds) so a pathological series cannot hang a request indefinitely; the request returns HTTP 504 with an actionable message.
