# Codex Guide, Tempolith

Tempolith is a free, Apache-2.0-licensed time-series forecasting workbench that
...
`tempolith-desktop` repo (Tauri shell + PyInstaller backend bundle). The
...
answer a question about the data, stop. Tempolith's only model is the
forecaster.

## Stack

- **Backend:** FastAPI + Uvicorn on port 8000. Entry:
  `app/backend/tempolith/main.py`. Model code: `src/timesfm/`.
...
- Backend CORS origins (`app/backend/tempolith/settings.py`) must include
  `http://localhost:5173`.
