...
Foreko V2 changes the product from a collection of analysis pages into a persistent forecasting workflow.
...
V2 preserves Foreko's existing boundaries:
...
The approved visual companion is stored under `.superpowers/brainstorm/foreko-v2-20260715/` and illustrates the intended information architecture.
...
Schedules are omitted from V2.0. Foreko does not show a disabled navigation destination for an unimplemented subsystem.
...
The default validation configuration is five rolling folds. The supported range is two to ten folds. If the available history cannot support the requested folds and horizon, Foreko proposes the largest valid fold count and requires user confirmation before running. It does not silently change the validation policy.
...
- If the optimizer fails, Foreko falls back to normalized inverse-MASE weights and records that fallback.
...
Calendar-generated factors may include month, quarter, day of week, weekend, and user-defined holiday flags. Foreko does not download holiday data automatically in V2.0.
...
2. Foreko freezes its values, intervals, assumptions, manifest, and export artifacts.
...
- File writes use atomic replacement and remain inside `FOREKO_STORAGE_DIR`.
...
- Foreko does not label confidence High solely because two models agree.
...
- All new persistent data resides under the configured Foreko storage directory.
