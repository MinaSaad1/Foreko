# Tempolith V2.0 Forecast Projects Implementation Plan (Revised)

> Supersedes `docs/superpowers/plans/2026-07-15-tempolith-v2-implementation.md`.
> Design of record remains `docs/superpowers/specs/2026-07-15-tempolith-v2-design.md`,
...
`~/.tempolith/projects/<project_id>/`. The React application becomes project-first through a
...
- `settings.py:87-88` `db_path` is a property resolving to `storage_dir/data/tempolith.db`.
...
**The e2e suite had a stale assertion and never ran.** `tempolith.spec.ts` asserted
...
on port 8000, so the journey created projects in real `~/.tempolith` storage and
...
- `app/backend/tempolith/services/migrations.py`: schema version detection and ordered migration steps.
...
- `app/backend/tempolith/services/ensemble_policy.py`: constrained out-of-fold ensemble weights and promotion guardrails. V2 only, independent of `ensembles.py` (see D2).
...
**Files:**
- Create: `app/backend/tempolith/services/migrations.py`
- Create: `app/backend/tests/unit/test_migrations.py`
- Modify: `app/backend/tempolith/services/store.py:105-107`
...
The first test is the one that matters. An existing user's `~/.tempolith/data/tempolith.db` has
...
- Modify: `app/backend/tempolith/services/migrations.py` (append migration 2)
- Modify: `app/backend/tempolith/services/store.py:298-305`
- Modify: `app/backend/tempolith/deps.py:44-51`
- Modify: `app/backend/tempolith/settings.py:98-107`
...
- Modify: `app/backend/tempolith/main.py:129-135` (CORS), `main.py:188-212` (router registration)
...
- Create: `app/backend/tempolith/routers/project_jobs.py`
...
- Modify: `app/backend/tempolith/services/backtest.py:45-48,153-260,291-309`
...
- Modify: `app/backend/tempolith/routers/project_jobs.py`
- Modify: `app/backend/tempolith/routers/system.py:64`
...
- Modify: `app/frontend/e2e/tempolith.spec.ts`
...
and leave the route and `comparison.py` untouched. Update `e2e/tempolith.spec.ts:89`, which asserts
...
entirely; Tempolith does not show a disabled destination for an unimplemented subsystem.

- [ ] **Step 3: Verify V1 regression**

Run: `cd app/frontend; npx playwright test e2e/tempolith.spec.ts --project=chromium`
...
deterministic but distinct fold predictions, gated by `TEMPOLITH_FAKE_MODELS=1`. The production
...
webServer: [
  {
    command: "uv run uvicorn tempolith.main:app --port 8000 --app-dir ../../app/backend",
    port: 8000,
    reuseExistingServer: !process.env.CI,
    timeout: 180_000,   // torch import is slow on a cold start
    env: {
      TEMPOLITH_FAKE_MODELS: "1",
      TEMPOLITH_STORAGE_DIR: process.env.TEMPOLITH_E2E_STORAGE_DIR!,
    },
  },
...
`TEMPOLITH_STORAGE_DIR` must point at a fresh empty directory per run, because the journey's first
...
suite with a temp `TEMPOLITH_E2E_STORAGE_DIR`. Upload `e2e/artifacts` on failure.
...
revisit if Tempolith adopts it. `rules-of-hooks` is an error, `exhaustive-deps` a warning, and tsc
remains the real gate on types.

That left 4 real errors, all dead code in `e2e/tempolith.spec.ts`: an uncalled `seedDatasetInApp`, an
...
Cache the model directory or rely on `TEMPOLITH_FAKE_MODELS=1` so CI performs no model download.
...
Update Privacy with every new SQLite table and `~/.tempolith/projects/` subdirectory.
...
- Evidence that V1 routes pass the existing `tempolith.spec.ts` smoke loop after the Task 9 nav change.
...
- A manual local smoke test using the Windows desktop packaging path, or a documented reason it
  must run in the separate `tempolith-desktop` repository.
...
- All new data lives under `TEMPOLITH_STORAGE_DIR`, and deleting a project removes its artifacts as well as its rows.
...
- The desktop packaging smoke test belongs to the separate `tempolith-desktop` repo and was not run here.
